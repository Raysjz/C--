...
int main() {  //------------Start of Main--------------------------------------------------------
  ...
  /* Debug output
    // If you try to access shapesArray[i] after deletion, it will be nullptr
      for (int i = 0; i < shapeCounter; i++) {
          if (shapesArray[i]) {
              cout << "Shape at index " << i << " still exists." << endl;
          } else {
              cout << "Shape at index " << i << " has been deleted." << endl;
          }
      }
  */
  return 0;
...
// Function to read user input and append the shape to the shape array
void inputSensorData() {
    string shapeName;
    string tempShape;
    string specialType;
    ...
    // Prompt user for the name of the shape
    cout << "Please enter name of shape : ";
    cin >> shapeName;
    tempShape = stringToUpper(shapeName);  // Convert input to uppercase for consistency
    cin.ignore(numeric_limits<streamsize>::max(), '\n');  // Clear the input buffer

    // Prompt the user for the special type with validation
    while (true) {
        cout << "Please enter special type : ";
        cin >> specialType;
        specialType = stringToUpper(specialType);  // Convert input to uppercase
        cin.ignore(numeric_limits<streamsize>::max(), '\n');  // Clear the input buffer

        // Check if the special type is valid
        if (specialType == "WS" || specialType == "NS") {
            containWarpSpace = (specialType == "WS");  // Set the boolean based on user input
    ...
    ShapeTwoD *newShape = nullptr;  // Pointer to hold the new shape object

    // Create the appropriate shape based on the user input after being changed to upperCase
    if (tempShape == "SQUARE") {
        shapeName = "Square";  // Sets shapeName for consistency
        newShape = new Square(shapeName, containWarpSpace, shapeCounter);
    } else if (tempShape == "RECTANGLE") {
        shapeName = "Rectangle";  // Sets shapeName for consistency
        newShape = new Rectangle(shapeName, containWarpSpace, shapeCounter);
    } else if (tempShape == "CIRCLE") {
        shapeName = "Circle";  // Sets shapeName for consistency
        newShape = new Circle(shapeName, containWarpSpace, shapeCounter);
    } else if (tempShape == "CROSS") {
        shapeName = "Cross";  // Sets shapeName for consistency
        newShape = new Cross(shapeName, containWarpSpace, shapeCounter);
    } else {
        // Handles invalid shape name
        cout << endl;
    ...
    // Add the newly created shape to the shapesArray
    if (newShape) {
        shapesArray[shapeCounter++] = newShape;  // Store the shape and increment the counter
        cout << endl;
...
void sortAscending(vector<ShapeTwoD *> Shapes) {
  if (Shapes.empty()) {
    cout << "[No shapes to sort, list is empty]" << endl;
...
void sortType(vector<ShapeTwoD *> Shapes) {
  ...
  specialNS.clear(); //Clear respective vectors
  specialWS.clear();
}
...
void displayExportmenu() {
  ...
  while (!validInput) {
    cout << "    a)      Export Shapes sorted by area (ascending)" << endl;
    cout << "    b)      Export Shapes sorted by area (descending)" << endl;
    cout << "    c)      Export Shapes sorted by special type and area" << endl;
    cout << endl;
...
void inputFilename(string &outputFile) {  // Bypass reference for filename
   if (shapeCounter == 0) { // Checks if ID = 0
...
void exportAscending(vector<ShapeTwoD *> Shapes, string filename) {
  ...
  */
  ...
  tempShapes.clear(); //Clear tempShapes.vector
}
...
void exportType(vector<ShapeTwoD *> Shapes, string filename) {
  ...
  */
  ...
  combined.clear(); // Clear respective vectors
  specialNS.clear();
  specialWS.clear();      
}
...
// Deletes Dynamic Memory of Shapes Array
void deallocateMemory() {
  for (int i = 0; i < shapeCounter; i++) {
