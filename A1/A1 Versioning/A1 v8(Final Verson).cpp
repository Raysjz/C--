...
Program Name : Weather Processing System C++11
*/
...
int choice; //Input Choice for processOption()
int x; // Array Border Size X-Coordinate
int y; // Array Border Size Y-Coordinate
...
vector<CityData> cities;        // Vector to store all city data from respective struct
vector<CloudData> clouds;       // Vector to store all cloud data from respective struct
vector<PressureData> pressures; // Vector to store all pressure data from respective struct
  ...
  switch (choice) {
    ...
    case 8:
      ...
      cout << "Thank you for using Weather Information Processing System, have "
              "a nice day!"
           << endl;
      //Clear 2DArray Dynamic Memory and Vectors containing cities , clouds and pressures upon exit     
      deallocateMemory(x, y);
      cities.clear(); 
      clouds.clear();
      pressures.clear();
...
void readConfig() {
  ...
  //Reset Values
  cities.clear(); //Clear Vectors containing cities , clouds and pressures to input new data into 2Darray
  clouds.clear();
  pressures.clear();
  ...
  //Reset Values for new inputs from another config file
  isArrayInitialized = true;
  int x = 0;   // Reset Array Border Size X-Coordinate to 0
  int y = 0;   // Reset Array Border Size Y-Coordinate to 0
  fileCounter = 0; //Reset File Counter to 0 
...
void readAFile(string& filename, int fileCounter) {
  ...
  if (!inputFile) {
    cerr << filename << "... [Failed to read file]" << endl;
    return;
  }else{
