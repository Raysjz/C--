...
void displayCitySummary(const vector<CityData>& cities,
    ...
    for (size_t i = 0; i < mediumCities.size(); ++i) {
        ...
        const string& cityName = mediumCityNames[i];
        const int cityNumber = mediumCityIDs[i];
        ...
        double avesCloud = ((surroundingCloud) / static_cast<double>(medCounter));
        double avesPressure = ((surroundingPressure) / static_cast<double>(medCounter));
