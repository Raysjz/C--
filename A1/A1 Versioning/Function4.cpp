...
void displayCitySummary(const vector<CityData>& cities,
    ...
    vector<pair<int, int>> mediumCities = extractCoordinates(cities, 4, mediumCityNames, mediumCityIDs);
    
    vector<string> bigCityNames;
    vector<int> bigCityIDs;
    vector<pair<int, int>> bigCities = extractCoordinates(cities, 9, bigCityNames, bigCityIDs);

    char pressureIndex = ' ';
    char cloudIndex = ' ';
    cout << "City Summary: " << endl;
    ...
    if (!smallCities.empty()) {
        for (size_t i = 0; i < smallCities.size(); i++) {
            ...
            smallCounter = 1; // Reset counter for each city
    ...
    // Process medium and big cities if needed
    // Similar logic can be applied for medium and big cities
}
