...
bool Point3D::operator<(const Point3D& other) const { //const ensures it's not modified while comparing
    if (x == other.x) {
        if (z == other.z) {
            return y < other.y;  // If z-coordinates are the same, compare y-coordinates
        }
        return z < other.z;  // If x-coordinates are the same, compare z-coordinates
    }
...
/* 
//This is changed in Final Version with greater<Point3D>() in Assn3
//Causing some difference in sorting for Point3D
