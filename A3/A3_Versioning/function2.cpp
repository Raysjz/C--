        ...
        if (classToken.size() > 0) {
			...
			if (type == "Point2D") {
                if (classToken.size() == 3) {
					...
					p2dVector.push_back(Point2D(x,y));
                    
                    cout << "\nProcessing Point2D with x: " << x << ", y: " << y << endl;
                } else {
                    cout << "Incorrect number of fields for Point2D record" << endl;
                }
            } else {
...
void readInput()
{
	...
	//cin >> inputFilename;
	inputFilename += "messy2.txt"; //Appending .txt to fileName for enhanced user experience
